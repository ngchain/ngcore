...
// A transact tx carries ONE calldata PER participant: the extra is
// rlp([][]byte) aligned with the participants, so a single tx can call
// a different entry with different args on every recipient contract.
// An empty extra means "no calldata for anyone".

var ErrTransactExtrasMisaligned = errors.New("transact extras do not align with the participants")

// EncodeTransactExtras packs the per-participant calldatas; all-empty
// canonicalizes to an empty extra
func EncodeTransactExtras(calldatas [][]byte) ([]byte, error) {
	allEmpty := true
	...
	return rlp.EncodeToBytes(calldatas)
}

// DecodeTransactExtras unpacks the per-participant calldatas of a
// transact tx with n participants
func DecodeTransactExtras(extra []byte, n int) ([][]byte, error) {
	if len(extra) == 0 {
		return make([][]byte, n), nil
	}

	var calldatas [][]byte
	if err := rlp.DecodeBytes(extra, &calldatas); err != nil {
		return nil, errors.Wrap(ErrTransactExtrasMisaligned, err.Error())
	}
	if len(calldatas) != n {
		return nil, errors.Wrapf(ErrTransactExtrasMisaligned, "%d calldatas for %d participants", len(calldatas), n)
	}
